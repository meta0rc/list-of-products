...
export const Filter = () => {
  ...
  const categoryFilter = categorys.filter(
    (product, index) => categorys.indexOf(product) === index
  );
  ...
  return (
    <FilterContainer>
      <ButtonFilter
        onClick={updateDrop}
        title={"Change any category to filter products"}
      >
        <p>
          {FilterContext.filter != "" ? FilterContext.filter : "Categorias"}
        </p>
        <BsChevronDown
          style={drop !== false ? buttonActive : buttonDesactive}
        />
      </ButtonFilter>
      {drop !== false && (
        <ListFilter>
